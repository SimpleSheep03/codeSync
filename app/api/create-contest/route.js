  ...
  try {
    ...
    while (newList.length < numQuestions) {
      if (Date.now() - startTime > timeLimitMillis) {
        return new Response(
          JSON.stringify({ message: "Time limit reached while fetching questions", ok: false }),
          { status: 408 }
        );
      }
    ...
    if (contestantType === 'Team' && contestants.length < 2) {
      return new Response(JSON.stringify({ message: "In team mode, provide at least 2 participants", ok: false }), { status: 400 });
    }
